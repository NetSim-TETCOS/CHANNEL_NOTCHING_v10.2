...
int fn_NetSim_CR_FormChannelSet(BS_PHY* pstruBSPhy)
{
	...
	for(nLoop=0;nLoop<nChannelCount;nLoop++)
	{
		...
		init_subchannel(nChannelCount,nChannelNumber);
#endif
...
#ifdef _CHANNEL_NOTCHING_
	init_scansubchannel(pstruBSMAC,pstruBSPHY);
#endif
	//Update the operating channel
	fn_NetSim_CR_UpdateOperatingChannel(pstruBSPHY);

#ifdef _CHANNEL_NOTCHING_
	updatesymbolparameter(pstruBSPHY->pstruOpratingChannel,pstruBSPHY->pstruSymbolParameter);
#endif
	return 1;
}
...
#ifdef _CHANNEL_NOTCHING_
				if(scansubchannel(getsubchannel(input->nChannelNumber),
					getLowerFrequency(input->nChannelNumber,input->pstruChannelList),
					getUpperFrequency(input->nChannelNumber,input->pstruChannelList),
